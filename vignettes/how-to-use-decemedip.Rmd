...
output: 
  ...
  BiocStyle::html_document:
    ...
    fig_height: 4.5
vignette: >
  %\VignetteIndexEntry{Analyzing methylated DNA immunoprecipitation sequencing data with decemedip}
...
```{r decemedip, fig.retina = NULL, fig.align='center', fig.wide = TRUE, echo=FALSE}
...
Note that our package provides default reference matrices for hg19 and hg38, along with the corresponding CpG density information. The objects can be accessed by calling `hg19.ref.cts.se`, `hg19.ref.anc.se` or `hg38.ref.cts.se`, `hg38.ref.anc.se`. 
...
paired <- TRUE # whether the sequeuncing 
sample_bam_file <- here(bam_dir, paste0(sample_name, '.bam'))
```
...
We use built-in objects of the package that contains read counts of the prostate tumor patient-derived xenograft (PDX) samples from the Berchuck et al. 2022 study, `pdx.counts.cts.se` and `pdx.counts.anc.se`, to demontrate the output and diagnostics in this vignette. 
...
```{r}
## Setting colors
CTS <- gsub('_', ' ', colnames(hg19.ref.cts.se))
CTCOLORS <- c(grafify:::graf_palettes$fishy[1:5], grafify:::graf_palettes$kelly)
names(CTCOLORS) <- CTS

## Plotting
smr_pi.df |>
  ggplot(aes(cell_type, mean, color = cell_type)) +
  ...
  geom_point(position = position_dodge2(width = 0.035),
             fill = 'white', shape = 21, size = 10) +
  # scale_fill_manual(values = CTCOLORS, labels = CTLABELS) +
  theme_classic() +
...
+ Berchuck JE, Baca SC, McClure HM, Korthauer K, Tsai HK, Nuzzo PV, et al. Detecting neuroendocrine prostate cancer through tissue-informed cell-free DNA methylation analysis. Clinical Cancer Research. 2022;28(5):928–938.
